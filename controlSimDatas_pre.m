classdef controlSimDatas_pre < handle
    properties
        ...
        simpleResults = struct('times',[], 'actRea',  [], 'egoAcc', [], 'actVel', [], 'actAcc', [], 'disInit', [],'DTC', [],'PET', [],'CriticalSpeed',[],'PETcondition',[],'CScondition',[],'isCollision', []);
        jsonDataRealtime
    ...
    methods
        ...
        function createSimpleResultStruct(obj,actRea,egoAcc,actVel,actAcc)
            ...
            obj.simpleResults.CriticalSpeed = obj.PET * 2 * obj.g * obj.f / 1000;

            if obj.PET <= 3000
                obj.simpleResults.PETcondition = 'risky';
            else
                obj.simpleResults.PETcondition = 'safety';
            end

            if obj.simpleResults.CriticalSpeed < 40
                obj.simpleResults.CScondition = 'risky';
            else
                obj.simpleResults.CScondition = 'safety';
            end

            % p = obj.PET;
            % obj.simpleResults.PETcondition = @(p)(p<=3000) * 'risky' + (p>3000) * 'safety';
            % p = obj.simpleResults.CriticalSpeed ;
            % obj.simpleResults.CSTcondition = @(p)(p<40) * 'risky' + (p>=40) * 'safety';
...
%                createJsonFile('realtime.json',obj.jsonDataRealtime)
        ...
        function CreateLogStructs(obj,egoVel,actVel,egoPos,actPos,isCollision,InitDis,egoAngVelLog,fieldName)
            ...
            obj.dataLog = struct(   'isCollision', '-', ...
                             'InitDis', InitDis, ...
            ...
            for i = 1:length(egoVel)
                ...
                if obj.dataLog.(fieldName)(i).actSpeed == 0 && obj.dataLog.(fieldName)(i).actAcc < 0 && obj.isActCompleted == false
                    obj.isActCompleted = true;
                ...
                if isCollision == true
                    obj.PET  =  0;
                    obj.dataLog.isCollision = 'Failed';
                else
                    obj.dataLog.isCollision = 'Success';
                end
