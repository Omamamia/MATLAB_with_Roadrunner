classdef controlSimDatas_forMovie < handle
    ...
    properties
        ...
        realtimeEgoData = struct('time', [], ...        % 現在時刻（シミュレーション時刻）
                        'egoSpeed', [] , ...    % Egoの速度（スカラー、normで計算）
                       'dis', [], ...         % EgoとActor間の距離（補正値rを考慮した値）
                      'isCollision', []);    % 現在の衝突状態（シミュレーション中の判定結果）
        realtimeActorData = struct('time', [], ...        % 現在時刻（シミュレーション時刻）
                        'actSpeed', [] , ...    % Egoの速度（スカラー、normで計算）
                       'dis', [], ...         % EgoとActor間の距離（補正値rを考慮した値）
                      'isCollision', []);    % 現在の衝突状態（シミュレーション中の判定結果）
    ...
    methods
        ...
        % -----------------------------------------------------------
        function obj = controlSimDatas_forMovie(rrSim)
        ...
        function setActor(obj,n)
 obj.act = Simulink.ScenarioSimulation.find("ActorSimulation", ActorID=uint64(n));
                    pause(0.01);  % 取得に成功した場合も少し待機して、システムの同期を図る
        end
        ...
        function creMovie(obj)
            obj.realtimeEgoData.time = obj.realtimeData.time ;
            obj.realtimeActorData.time = obj.realtimeData.time ;

             obj.realtimeActorData.actSpeed =obj.realtimeData.actSpeed ;
           

                      obj.realtimeEgoData.dis = obj.realtimeData.dis;
            obj.realtimeActorData.dis = obj.realtimeData.dis;

             obj.realtimeEgoData.isCollision = obj.realtimeData.isCollision;
            obj.realtimeActorData.isCollision = obj.realtimeData.isCollision;

        end
        function createRealtimeStructs(obj)
            % 現在のシミュレーション時刻を取得（rrSimオブジェクトから直接取得）
            obj.realtimeData.time  = get(obj.rrSim, "SimulationTime") * 1000;
            ...
            obj.realtimeEgoData.egoSpeed =obj.realtimeData.egoSpeed ;
        ...
        function createLogStructs(obj)
            ...
            actVel = get(simLog, 'Velocity', 'ActorID', 3);

            % 同様に、位置情報（Pose）も各Actorごとに抽出
            egoPos = get(simLog, "Pose", "ActorID", 1);
            actPos = get(simLog, "Pose", "ActorID", 3);
