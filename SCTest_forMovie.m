try  
    ...
    dcaseID = 'egSWwlqanC8nxYkAyTLAvB4Y1Sy7w69j1lV71y6J_LY_'; % D-Caseを識別するID
    ...
    realTimeDataJsonName = 'realtime_forMovie.json'; % シミュレーション中のリアルタイムデータを保存するJSONファイル名
    logDataJsonName = 'resultTest.json';        % シミュレーション全体のログデータを保存するJSONファイル名
    resultsJsonName = 'resultSimpletest.json';  % 最終結果をまとめたJSONファイル名
    resultCsvName = 'outputTest_forMovie.csv';           % 最終結果をCSV形式で保存するためのファイル名
    ...
    scenarioFile = "/home/furuuchi/ドキュメント/GitHub/Roadrunner/Scenarios/Testcase_pre2_forMovie.rrscenario";
    ...
    simulationPace = 1;       % シミュレーション実行の速度倍率（例：20倍速で実行）
    ...
    % 最終結果の算出などを担う
    simDatas = controlSimDatas_forMovie(rrSim);
        ...
        for n = 1
            ...
            while strcmp(get(rrSim, "SimulationStatus"), "Running")
                try
                    ...
                    savedDataActor1 = simDatas.realtimeActorData;
                    
                    % JSON形式の文字列を指定ファイルに保存する（上書き保存）
                    createJsonFile(realTimeDataJsonName, sendData);
                    createJsonFile('actor1Data.json', jsonencode(savedDataActor1));
                    ...
                    % 取得したリアルタイムデータ構造体をJSON形式の文字列に変換する
                    sendData = jsonencode(simDatas.realtimeData);
                    savedDataActor2 = simDatas.realtimeActorData;
                    
                    % JSON形式の文字列を指定ファイルに保存する（上書き保存）
                    createJsonFile(realTimeDataJsonName, sendData);
                    createJsonFile('actor2Data.json', jsonencode(savedDataActor2));
            ...
            % -----------------------------------------------------------
            savedDataActor1.time = 15 * 1000;
            savedDataActor1.isCollision = 'Success';
            createJsonFile('actor1Data.json', jsonencode(savedDataActor1));
            savedDataActor2.time = 15 * 1000;
            savedDataActor2.isCollision = 'Success';
            createJsonFile('actor2Data.json', jsonencode(savedDataActor2));
            ...
            simDatas.realtimeData = sendData;
